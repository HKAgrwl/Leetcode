...
public:
    string s1, s2;
    vector<vector<int>> dp; //brain of program
    int util(int i, int j){
        if(i>=s1.size() || j>=s2.size())return 0; //if any index goes out of bound then we should return

        if(dp[i][j]!= -1)return dp[i][j]; //if our brain cells already filled then is there any need to calculate same task again.
    ...
    int longestCommonSubsequence(string text1, string text2) {
        s1= text1, s2= text2;
        dp.resize(text1.size()+1, vector<int>(text2.size()+1, -1));//defining our brain size shabbily.
        return util(0, 0);
    }
};